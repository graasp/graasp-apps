...
import { Item, ItemSettings } from 'graasp';
...
import common, { generateToken, getContext, patchSettings } from './schemas';
...
const plugin: FastifyPluginAsync<AppsPluginOptions> = async (fastify, options) => {
  ...
  fastify.register(async function (fastify) {
    ...
    fastify.register(async function (fastify) {
      ...
      fastify.patch<{ Params: { itemId: string }; Body: ItemSettings }>(
        '/:itemId/settings',
        {
        ...
        async (
          { member, authTokenSubject: requestDetails, params: { itemId }, body, log },
          reply,
        ) => {
          const memberId = member ? member.id : requestDetails.member;

          // check token validity if token is available
          // if the token is not available, the remaining process will check depending on the regular member
          if (requestDetails) {
            const { item: tokenItemId } = requestDetails;
            checkTargetItemAndTokenItemMatch(itemId, tokenItemId);
          }
          // this sequence will check the member permission over the item
          const tasks = iTM.createUpdateTaskSequence({ id: memberId }, itemId, { settings: body });
